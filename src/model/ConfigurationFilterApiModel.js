/**
 * API
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: v2.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';

/**
 * The ConfigurationFilterApiModel model module.
 * @module model/ConfigurationFilterApiModel
 * @version 7.0.0
 */
class ConfigurationFilterApiModel {
    /**
     * Constructs a new <code>ConfigurationFilterApiModel</code>.
     * @alias module:model/ConfigurationFilterApiModel
     */
    constructor() { 
        
        ConfigurationFilterApiModel.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
    }

    /**
     * Constructs a <code>ConfigurationFilterApiModel</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/ConfigurationFilterApiModel} obj Optional instance to populate.
     * @return {module:model/ConfigurationFilterApiModel} The populated <code>ConfigurationFilterApiModel</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new ConfigurationFilterApiModel();

            if (data.hasOwnProperty('projectIds')) {
                obj['projectIds'] = ApiClient.convertToType(data['projectIds'], ['String']);
            }
            if (data.hasOwnProperty('name')) {
                obj['name'] = ApiClient.convertToType(data['name'], 'String');
            }
            if (data.hasOwnProperty('isDeleted')) {
                obj['isDeleted'] = ApiClient.convertToType(data['isDeleted'], 'Boolean');
            }
            if (data.hasOwnProperty('globalIds')) {
                obj['globalIds'] = ApiClient.convertToType(data['globalIds'], ['Number']);
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>ConfigurationFilterApiModel</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>ConfigurationFilterApiModel</code>.
     */
    static validateJSON(data) {
        // ensure the json data is an array
        if (!Array.isArray(data['projectIds'])) {
            throw new Error("Expected the field `projectIds` to be an array in the JSON data but got " + data['projectIds']);
        }
        // ensure the json data is a string
        if (data['name'] && !(typeof data['name'] === 'string' || data['name'] instanceof String)) {
            throw new Error("Expected the field `name` to be a primitive type in the JSON string but got " + data['name']);
        }
        // ensure the json data is an array
        if (!Array.isArray(data['globalIds'])) {
            throw new Error("Expected the field `globalIds` to be an array in the JSON data but got " + data['globalIds']);
        }

        return true;
    }


}



/**
 * Collection of identifiers of projects from which configurations will be taken
 * @member {Array.<String>} projectIds
 */
ConfigurationFilterApiModel.prototype['projectIds'] = undefined;

/**
 * Filter to search by name (case-insensitive, partial match)
 * @member {String} name
 */
ConfigurationFilterApiModel.prototype['name'] = undefined;

/**
 * Is configurations deleted or existing
 * @member {Boolean} isDeleted
 */
ConfigurationFilterApiModel.prototype['isDeleted'] = undefined;

/**
 * Collection of global (integer) identifiers to filter configurations
 * @member {Array.<Number>} globalIds
 */
ConfigurationFilterApiModel.prototype['globalIds'] = undefined;






export default ConfigurationFilterApiModel;

